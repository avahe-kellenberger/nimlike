...
task runr, "Runs nimlike for current platform":
 exec "nim c --multimethods:on -r -d:release -o:nimlike src/main.nim"

task rund, "Runs debug nimlike for current platform":
 exec "nim c --multimethods:on -r -d:debug -o:nimlike src/main.nim"

task release, "Builds nimlike for current platform":
 exec "nim c --multimethods:on -d:release -o:nimlike src/main.nim"

task debug, "Builds debug nimlike for current platform":
 exec "nim c --multimethods:on -d:debug -o:nimlike_debug src/main.nim"

task web, "Builds nimlike for current web":
 exec "nim js --multimethods:on -d:release -o:nimlike.js src/main.nim"

task webd, "Builds debug nimlike for current web":
 exec "nim js --multimethods:on -d:debug -o:nimlike.js src/main.nim"
